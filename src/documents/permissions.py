...
def set_permissions_for_object(permissions: list[str], object, merge: bool = False):
    ...
    existing_user_ids = []  # List to store existing user IDs with permissions
    new_user_ids = set()    # Set to store new unique user IDs being added
    ...
    for action in permissions:
        ...
        existing_user_ids.extend(users_to_remove.values_list('id', flat=True))  # Add existing user IDs to the list
        ...
        if len(users_to_add) > 0:
            for user in users_to_add:
                if user.id not in existing_user_ids:  # Check if the user is new
                    new_user_ids.add(user.id)  # Using a set to automatically remove duplicates
                ...
                new_user_id_array = list(new_user_ids)  # Convert set to list

                print("New User IDs without permissions:", new_user_id_array)

                # Set new user IDs as environment variables
                os.environ["DOCUMENT_USER_ID"] = json.dumps(new_user_id_array)
